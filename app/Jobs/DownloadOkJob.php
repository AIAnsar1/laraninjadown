...
class DownloadOkJob implements ShouldQueue
{
     ...
     * Execute the job.
     */
    public function handle(Nutgram $bot, OkService $ok_service)
    {
        $lang = $bot->user()?->language ?? 'ru';
        $caption = __('Messages.instagram_video_downloaded', [], $lang);
        $result = $ok_service->download($this->url);
