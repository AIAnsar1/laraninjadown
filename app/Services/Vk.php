...
class Vk extends BaseService
{
    ...
    /**
     * Скачать медиа из VK (видео, клипы, музыка)
     * @param string $url
     * @return array|false
     */
    public function download(string $url): array|false
    {
        $outputPath = storage_path('app/vk');
        ...
        // Определим тип: video | clip | music
        $type = match (true) {
            str_contains($url, 'vk.com/clip') => 'clip',
            str_contains($url, 'vk.com/music') => 'music',
            str_contains($url, 'vk.com/video') => 'video',
            default => 'unknown',
        };
