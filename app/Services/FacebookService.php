...
class FacebookService extends BaseService
{
    ...
    /**
     * Скачать любое видео из Facebook (включая reels, stories, etc.)
     * @param string $url
     * @return array|false
     */
    public function download(string $url): array|false
    {
        $outputPath = storage_path('app/facebook');
        ...
        // Определим тип: video | reel | story | unknown
        $type = match (true) {
            str_contains($url, 'facebook.com/reel') => 'reel',
            str_contains($url, 'facebook.com/story') => 'story',
            str_contains($url, 'facebook.com/watch') || str_contains($url, 'facebook.com/') => 'video',
            default => 'unknown',
        };
